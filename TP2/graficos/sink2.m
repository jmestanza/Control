...
f = csvread('escalonSinK2.csv',2,0); % ignoro los primeros 2 renglones 
tmed = f(:,1);
vimed = f(:,2);
vomed = f(:,3);

medstart = find(vimed > 0, 1, 'first'); % de mirar la medicion esto da
...
simend = find(tsim>=1, 1, 'first')-1;
medend = find(tmed>=1, 1, 'first')-1;
